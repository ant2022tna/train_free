...
async def main(args):
    ...
    if args.domain == "medical":
        ...
        from training_free_grpo.medical.prompts import PROBLEM_WITH_EXPERIENCE_TEMPLATE, PROBLEM_WITH_EXPERIENCE_AND_EXPERT_TEMPLATE, DIVERSE_EXPERTS_PERSONAS, GENERATE_MEDICAL_FIELDS_TEMPLATE
    ...
    if os.path.exists(stats_filename):
        ...
        stats = {}
        stats["arg"]= {}
        stats["arg"]["mode"] = args.mode
    stats["arg"]["domain"] = args.domain
    stats["arg"]["experiment_name"] = args.experiment_name
    stats["arg"]["dataset"] = args.dataset
    stats["arg"]["dataset_truncate"] = args.dataset_truncate
    stats["arg"]["given_ground_truth"] = args.given_ground_truth
    stats["arg"]["epochs"] = args.epochs
    stats["arg"]["batchsize"] = args.batchsize
    stats["arg"]["grpo_n"] = args.grpo_n
    stats["arg"]["rollout_concurrency"] = args.rollout_concurrency
    stats["arg"]["rollout_temperature"] = args.rollout_temperature
    stats["arg"]["rollout_max_tokens"] = args.rollout_max_tokens
    stats["arg"]["task_timeout"] = args.task_timeout
    stats["arg"]["use_trajectory_summary_for_synthesis"] = args.use_trajectory_summary_for_synthesis
    stats["arg"]["use_diverse_experts"] = args.use_diverse_experts
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            if args.use_generated_experts:
                print(f"Generating domain experts for {len(batch_data)} problems...")
                prompts_for_domain_gen = [
                    GENERATE_MEDICAL_FIELDS_TEMPLATE.format(
                        problem=each['problem'],
                        num_experts=args.grpo_n
                    ) for each in batch_data
                ]
                
                domain_strings = await llm.chat_batch_async(
                    prompts=prompts_for_domain_gen,
                    concurrency=args.rollout_concurrency
                )
                
                problem_to_personas = {}
                for i, d_str in enumerate(domain_strings):
                    try:
                        clean_str = d_str.strip()
                        if clean_str.startswith("Medical Field:"):
                            clean_str = clean_str[len("Medical Field:"):].strip()
                        
                        fields = [field.strip() for field in clean_str.split('|') if field.strip()]
                        
                        if fields:
                            personas = [f"You are a medical expert specializing in {field}. You will solve the problem from this perspective." for field in fields]
                            problem_to_personas[batch_data[i]['problem']] = personas
                        else:
                            print(f"Warning: Could not parse medical fields for problem {i} from response: '{d_str}'. Using fallback.")
                            problem_to_personas[batch_data[i]['problem']] = DIVERSE_EXPERTS_PERSONAS
                    except Exception as e:
                        print(f"Warning: Error processing medical fields for problem {i}: {e}. Using fallback.")
                        problem_to_personas[batch_data[i]['problem']] = DIVERSE_EXPERTS_PERSONAS

            if args.use_generated_experts:
                print(f"Using generated diverse experts for GRPO rollout number={args.grpo_n}")
                formatted_batch_data = []
                for each in batch_data:
                    personas = problem_to_personas.get(each['problem'], DIVERSE_EXPERTS_PERSONAS)
                    if not personas:
                        personas = DIVERSE_EXPERTS_PERSONAS
                    
                    for i in range(args.grpo_n):
                        expert_persona = personas[i % len(personas)]
                        prompt = PROBLEM_WITH_EXPERIENCE_AND_EXPERT_TEMPLATE.format(
            ...
            elif args.use_diverse_experts:
                print(f"Using diverse experts for GRPO rollout number={args.grpo_n}")
                formatted_batch_data = []
                for each in batch_data:
                    for i in range(args.grpo_n):
                        expert_persona = DIVERSE_EXPERTS_PERSONAS[i % len(DIVERSE_EXPERTS_PERSONAS)]
                        if experiences:
                            prompt = PROBLEM_WITH_EXPERIENCE_AND_EXPERT_TEMPLATE.format(
                                expert_persona=expert_persona,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--domain", type=str, required=True, choices=["medical"], help="domain of the tasks (math/web/medical)")
    ...
    parser.add_argument("--rollout_max_tokens", type=int, default=8192, help="Max tokens for each rollout batch")
    ...
    parser.add_argument("--use_diverse_experts", action="store_true", help="Use diverse experts for GRPO rollouts.")
    parser.add_argument("--use_generated_experts", action="store_true", help="Use LLM-generated diverse experts for GRPO rollouts.")
